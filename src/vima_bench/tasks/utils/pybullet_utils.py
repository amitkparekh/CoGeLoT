...
def get_urdf_path(
    ...
    if obj_entry.from_template:
        ...
        assert obj_entry.size_range.low == obj_entry.size_range.high, (
            "Try to add an object not from a template. "
            "In this case its size is fixed and expect `size_range.low` = `size_range.high`, "
...
def add_any_object(
    ...
    :param replace: Key strings to be replaced. If None, will first try to use obj_entry.replace_fn
        and then try replace size only.
    :param retain_temp: Keep the generated temporal file or not if the object is added from a
        template.
    ...
    if obj_entry.pose_transform_fn is not None and pose[0] is not None and pose[1] is not None:
...
def add_object_id_reverse_mapping_info(mapping_dict, obj_id, object_entry, texture_entry):
    """
...
def set_visibility_bullet(pybullet_client_id, object_id, alpha_value):
    ...
    """
    visual_shape_list = p.getVisualShapeData(object_id, physicsClientId=pybullet_client_id)
    for visual_shape in visual_shape_list:
        object_id, link_idx, _, _, _, _, _, rgba_color = visual_shape
        p.changeVisualShape(object_id, link_idx, rgbaColor=list(rgba_color)[:3] + [alpha_value])
...
def is_one_object_in_a_hollow_2d_object(
    ...
    diff_pos = np.float32(test_object_pose[0][:2]) - np.float32(hollow_object_pose[0][:2])
...
def if_in_hollow_object(
    ...
    if not isinstance(hollow_object_size, list):
        ...
        hollow_object_size = [hollow_object_size for _ in range(len(hollow_object_poses))]
...
def p_change_texture(obj_id: int, texture_entry: TextureEntry, client_id: int):
    assert not (texture_entry.color_value is None and texture_entry.texture_asset is None)
