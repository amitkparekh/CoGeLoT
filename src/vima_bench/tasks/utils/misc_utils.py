...
def sample_distribution(prob, rng, n_samples=1):
    ...
    rand_ind = rng.choice(np.arange(len(flat_prob)), n_samples, p=flat_prob, replace=False)
...
def check_transform(image, pixel, transform):
    ...
    new_pixel = np.flip(
        np.int32(np.round(np.dot(transform, np.float32([pixel[1], pixel[0], 1.0]).reshape(3, 1))))[
            :2
        ].squeeze()
    )
    valid = (
        np.all(new_pixel >= 0) and new_pixel[0] < image.shape[0] and new_pixel[1] < image.shape[1]
    )
