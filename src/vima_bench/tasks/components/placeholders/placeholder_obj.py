...
class PlaceholderObj(Placeholder):
    ...
    def _render_camera(self, config, image_size=None, client_id=None):
        ...
        viewm = p.computeViewMatrix(config["position"], lookat, updir, physicsClientId=client_id)
        ...
        for each_type in types:
            if each_type == "image":
                ...
                for view_name, view_config in self.image_views.items():
                    rgb, seg = self._render_camera(view_config, client_id=obj_showing_sim_id)
                ...
                expressions["alias"] = f"{self.color.name} {alias}" if prepend_color else alias
