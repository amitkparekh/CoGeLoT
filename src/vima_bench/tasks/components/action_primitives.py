    ...
    def __call__(self, movej, movep, ee, target_pose, release: bool):
        ...
        while abs(ee.cur_pose[0][2] - target_pose[0][2] > self._delta_z):
            delta = (
                np.float32(
                    [
                        0,
                        0,
                        -self._delta_z if ee.cur_pose[0][2] > target_pose[0][2] else self._delta_z,
                    ]
                ),
            ...
            if n_adjustments > self._max_adjust:
                print(f"Warning: height adjustments exceeded {self._max_adjust} times. Skipping.")
                break
            n_adjustments += 1
...
class MoveEndEffectorForPush(object):
    ...
    def __call__(self, movej, movep, ee, target_pose, push: bool):
        ...
        if not push:
            ...
            pos1 = np.float32((target_pose[0][0], target_pose[0][1], self._operation_height))
...
class Push:
    def __init__(self, rest_height=0.31, operation_height=0.005, speed=0.002, step_size=0.001):
        self._rest_height = rest_height
        self._operation_height = operation_height
        self._speed = speed
        self._step_size = step_size
