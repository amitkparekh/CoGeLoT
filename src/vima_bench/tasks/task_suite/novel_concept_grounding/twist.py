...
class Twist(RotateTheObjBase):
    """Novel concept (verbs) grounding: Object Twisting Tasks The agent needs to learn from the
    prompt how is "twist" defined, and what is the exact angle to twist.

    Like the ring balancing tasks, it needs to ground verbs that are semantically similar but
    different in exact definitions.
    """
    ...
    def __init__(
        ...
        if possible_angles_of_rotation is None:
            ...
            self.possible_angles_of_rotation = [1 / 6 * math.pi * i for i in range(1, 12)]
    ...
    def _reset_prompt(self, *args, **kwargs):
        self.task_meta["num_dragged_obj"] = self.rng.integers(1, self.task_meta["num_total_objs"])
    ...
    def reset(self, env, *args, **kwargs):
        ...
        for i in range(self._n_supports):
            ...
            sampled_example_obj_texture = self.rng.choice(self.possible_dragged_obj_texture).value
            ...
            def create_scene_fn(
                ...
                # the start scene
                if scene_name.startswith("before_twist"):  # generate with placement configurations
                    # Add the example obj.
