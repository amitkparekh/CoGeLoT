...
class NovelAdj(BaseTask):
    ...
    def __init__(
        ...
        if novel_adjectives is None:
            ...
            for comp in novel_adjectives:
                assert isinstance(comp, str), f"{comp} has to be string in novel adjectives"
        ...
        if adjective_types is None:
            ...
            for comp in adjective_types:
                assert isinstance(comp, str), f"{comp} has to be string in adjective_types"
        ...
        if possible_dragged_obj is None:
            ...
            self.possible_dragged_obj = [ObjPedia.lookup_object_by_name(possible_dragged_obj)]
            ...
            if isinstance(possible_dragged_obj[0], str):
                ...
                raise ValueError("possible_dragged_obj must be a list of str or ObjEntry")
        else:
            raise ValueError("possible_dragged_obj must be a str or list of str or ObjEntry")
        ...
        if possible_base_obj is None:
            ...
            raise ValueError("possible_base_obj must be a str or list of str or ObjEntry")
        ...
        if possible_dragged_obj_texture is None:
            ...
            if isinstance(possible_dragged_obj_texture[0], str):
                self.possible_dragged_obj_texture = [
                    TexturePedia.lookup_color_by_name(obj) for obj in possible_dragged_obj_texture
                ]
        ...
        if possible_base_obj_texture is None:
            ...
            if isinstance(possible_base_obj_texture[0], str):
                self.possible_base_obj_texture = [
                    TexturePedia.lookup_color_by_name(obj) for obj in possible_base_obj_texture
                ]
            elif isinstance(possible_base_obj_texture[0], TextureEntry):
                self.possible_base_obj_texture = possible_base_obj_texture
            else:
                raise ValueError("possible_base_obj_texture must be a list of str or TextureEntry")
        else:
            raise ValueError(
        ...
        for i in range(n_supports):
            ...
            prompt_template += f"{_blicker_str} is {_novel_adj_str} than {_less_blicker_str}. "
    ...
    def reset(self, env):
        ...
        sampled_dragged_obj_color = self.rng.choice(self.possible_dragged_obj_texture).value
        if adjective_definition == "lighter color":
            dragged_color = TexturePedia.find_lighter_variant(sampled_dragged_obj_color)
            confusion_color = TexturePedia.find_darker_variant(sampled_dragged_obj_color)
        elif adjective_definition == "darker color":
            dragged_color = TexturePedia.find_darker_variant(sampled_dragged_obj_color)
            confusion_color = TexturePedia.find_lighter_variant(sampled_dragged_obj_color)
        else:
        ...
        for i in range(self._n_supports):
            sampled_demo_obj_color = self.rng.choice(self.possible_dragged_obj_texture).value

            if adjective_definition == "lighter color":
                demo_blicker_color = TexturePedia.find_lighter_variant(sampled_demo_obj_color)
                demo_less_blicker_color = TexturePedia.find_darker_variant(sampled_demo_obj_color)
            elif adjective_definition == "darker color":
                demo_blicker_color = TexturePedia.find_darker_variant(sampled_demo_obj_color)
                demo_less_blicker_color = TexturePedia.find_lighter_variant(sampled_demo_obj_color)
            else:
        ...
        for i in range(self.REJECT_SAMPLING_MAX_TIMES):
            ...
            dragged_obj_scaled_size = self._scale_size(dragged_obj_size, dragged_obj_scalar)
        ...
        for i in range(self.REJECT_SAMPLING_MAX_TIMES):
            # add first confusion object
            confusion1_scaled_size = self._scale_size(dragged_obj_size, confusion_obj_scalar)
        ...
        if self.task_meta["difficulty"] in ["medium", "hard", "distracting"]:
            ...
            for i in range(self.REJECT_SAMPLING_MAX_TIMES):
                # add first confusion object
                distractor_scaled_size = self._scale_size(distractor_size, distractor_size_scalar)
        ...
        for i in range(
            ...
            obj_id, urdf_full_path, _ = self.add_object_to_env(
                ...
                category="fixed" if distractor_obj in self.possible_base_obj else "rigid",
                retain_temp=True,
            )
            if obj_id is None:
                print(f"Warning: {i + 1} repeated sampling when try to spawn distractors")
    ...
    def check_success(self, *args, **kwargs) -> ResultTuple:
        ...
        if all_achieved:
            ...
            obj_id, _ = obj[0]
            obj_pose = p.getBasePositionAndOrientation(obj_id, physicsClientId=self.client_id)
            ...
            result = ResultTuple(
                success=False,
                failure=is_failure,
                distance=np.linalg.norm(np.float32(obj_pose[0][:2]) - np.float32(base[0][0][:2])),
            )
            return result
