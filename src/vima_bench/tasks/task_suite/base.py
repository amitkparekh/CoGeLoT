...
class BaseTask:
    ...
    def set_difficulty(self, difficulty: str):
        """Set difficulty for different evaluation schemes. Note that this method is supposed to be
        only called ONCE, because implementations of most subclasses involve reuse "states" that
    ...
    def oracle(self, env):
        ...
        def act(obs):
            ...
            for i in range(len(objs)):
                object_id, (symmetry, _) = objs[i]
                xyz, _ = p.getBasePositionAndOrientation(object_id, physicsClientId=self.client_id)
                targets_i = np.argwhere(matches[i, :]).reshape(-1)
                if len(targets_i) > 0:
    ...
    def update_goals(self, skip_oracle=True):
        ...
        One goal each time, E.g., original goal sequence (A, B), after successfully achieving goal
        A, the sequence will be updated to (B).
        """
    ...
    def check_success(self, *args, **kwargs) -> NamedTuple:
        ...
        It should return a tuple of two boolean values, (success, failure). A trajectory will be
        terminated if fails. This function may be invoked at the final step. It may also be invoked
        every step for "constraint satisfaction" tasks.
        """
    ...
    def check_constraint(self, *args, **kwargs):
        ...
        This should be implemented by constraint satisfaction tasks. The result of constraint
        checking should be stored and used to determine episode success in check_success() later
        """
        return
    ...
    def generate_prompt(self, *args, **kwargs) -> Any:
        ...
        This method may be invoked in `env.reset()`. Implementation of this method may vary in
        different tasks.
        """
        expressions = {}
