...
class SweepObjectsToZoneBase(BaseTask):
    """Base class for tasks that sweep objects into a zone Shared helper functions for constraint
    checking."""
        ...
        for idx, texture in enumerate(texture_list):
            if texture is None:
                ...
                if isinstance(texture[0], str):
                    texture_list[idx] = [TexturePedia.lookup_color_by_name(obj) for obj in texture]
    ...
    def is_touching_zone(
        ...
        percentage_in_zone = self.check_percentage_in_zone(obj_id, pts, zone, zone_with_margin)
    ...
    def check_constraint(self):
        ...
        for obj_id in obj_pts.keys():
            if self.is_touching_zone(obj_id, obj_pts[obj_id], constraint_zone, zone_with_margin):
    ...
    def check_success(self, *args, **kwargs) -> ResultTuple:
        ...
        if self.constraint_checking["enabled"] and not self.constraint_checking["satisfied"]:
