...
class WithoutTouching(SweepObjectsToZoneBase):
    ...
    def reset(self, env):
        ...
        probs = [self.task_meta["sample_prob"][i] for i in sorted(self.task_meta["sample_prob"])]
        ...
        sampled_constraint_obj_texture = self.rng.choice(
            [e for e in self.possible_base_obj_texture if e.value != sampled_base_obj_texture]
        ).value

        sampled_swept_obj_born_pos, sampled_distractor_obj_born_pos = [
            e for e in self.rng.choice(self.possible_swept_obj_born_pos, size=2, replace=False)
        ]

        sampled_distractor_obj_texture = self.rng.choice(
            [e for e in self.possible_dragged_obj_texture if e.value != sampled_swept_obj_texture]
        ).value
        ...
        constraint_pos_x = (
            base_pos_x + 0.5 * base_obj_sampled_size[0] + 0.5 * constraint_obj_sampled_size[1]
        )
        ...
        p_change_texture(constraint_obj_id, sampled_constraint_obj_texture, self.client_id)
        add_object_id_reverse_mapping_info(
            mapping_dict=env.obj_id_reverse_mapping,
        ...
        # sample determiner
        dets = [k for k, v in det_to_integer.items() if v <= sampled_num_swept_objs] + ["all"]
    ...
    def oracle(self, env):
        def act(obs):
            ...
            for i in range(len(objs)):
                ...
                xyz, _ = getBasePositionAndOrientation(object_id, physicsClientId=self.client_id)
                obj_xyz.append(xyz)
                ...
                if self.phases["left_divider_y"] < xyz[1] < self.phases["right_divider_y"]:
            ...
            for ph in [3, 2, 1]:
                ...
                sorted_phase_obj = [
                    x for _, x in sorted(zip(phase_dists, phase_obj), key=lambda pair: -pair[0])
                ]
