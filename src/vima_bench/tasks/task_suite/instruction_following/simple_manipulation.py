    ...
    def reset(self, env):
        ...
        for i in range(self.REJECT_SAMPLING_MAX_TIMES):
            ...
            if obj_id is not None:
                ...
                # add placeholder objects
                self.placeholders[f"dragged_obj_{n_added_dragged_obj + 1}"] = PlaceholderObj(
        ...
        _distracting_combos = [
            r for r in itertools.product(possible_distractor_obj, possible_distractor_texture)
        ]
        distracting_combos = []
        # remove used combo in dragg and base obj
        for combo in _distracting_combos:
            if (combo[0].value == sampled_base_obj and combo[1].value == sampled_colors[0]) or (
                self._exclude_distractor_by_geometry
                and (combo[0].value == sampled_base_obj or combo[0].value == sampled_dragged_obj)
            ):
                pass
            elif (
                combo[0].value == sampled_dragged_obj and combo[1].value in sampled_colors[1]
            ) or (
                self._exclude_distractor_by_geometry
                and (combo[0].value == sampled_dragged_obj or combo[0].value == sampled_base_obj)
            ):
                pass
            else:
        ...
        self.distractors = []
        not_reach_max_times = False
        for i in range(self.task_meta["num_distractors_obj"] + self.REJECT_SAMPLING_MAX_TIMES):
