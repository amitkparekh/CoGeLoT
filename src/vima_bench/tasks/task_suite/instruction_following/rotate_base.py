...
class RotateTheObjBase(BaseTask, ABC):
    ...
    def __init__(
        ...
        if possible_angles_of_rotation is None:
            ...
            self.possible_angles_of_rotation = [1 / 6 * math.pi * i for i in range(1, 6)]
    ...
    def _reset_prompt(self, *args, **kwargs):
        self.sampled_angle_of_rotation = self.rng.choice(self.possible_angles_of_rotation)
        self.sampled_dragged_obj_texture = self.rng.choice(self.possible_dragged_obj_texture).value

    def reset(self, env, same_dragged_obj: bool = True, same_dragged_color: bool = True):
        super().reset(env)
        ...
        for i in range(self.task_meta["num_dragged_obj"] + self.REJECT_SAMPLING_MAX_TIMES):
            if same_dragged_obj:
        ...
        for i in range(self.task_meta["num_distractors"] + self.REJECT_SAMPLING_MAX_TIMES):
