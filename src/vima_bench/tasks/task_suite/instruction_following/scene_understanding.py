...
class SceneUnderstanding(BaseTask):
    ...
    def __init__(
        ...
        for num in [
            ...
            assert num <= 3, (
                "The current size of the workspace only supports the max number of each "
        ...
        for expression, types in placeholder_expression.items():
            if types == "name":
                placeholder_expression[expression]["prepend_color"] = prepend_color_to_name
    ...
    def reset(self, env):
        ...
        def create_scene_fn(scene_render_env: SceneRenderEnv, sim_id: int):
            ...
            for dragged_obj_entry, sampled_size in zip(
                ...
                pose = scene_render_env.renderer_get_random_pose(sampled_size, rng=self.rng)
                obj_id, _ = add_any_object(
            ...
            for entry, texture in zip(sampled_distractor_entries, sampled_distractor_texture):
        ...
        not_reach_max_times = False
        for i in range(
            self.task_meta["num_distractor_in_workspace"] + 2 * self.REJECT_SAMPLING_MAX_TIMES
        ):
    ...
    def check_success(self, *args, **kwargs) -> ResultTuple:
        ...
        for obj_id in current_obj_goal_map.keys():
            if current_obj_goal_map[obj_id] == self.task_meta["num_base_obj"] - 1:
                need_to_remove = [goal for goal in self.goals if goal[0][0][0] == obj_id]
