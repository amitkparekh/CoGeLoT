...
class Rotate(RotateTheObjBase):
    """Task on rotating objects: rotation around z-axis Previously we care most about achieving
    target (x-y) positions for our tasks. In this task, rotation is introduced by let the robot arm
    to rotate objects by certain degrees (either clockwise or counterclockwise). An angle of
    rotational symmetry (the smallest angle for which the figure can be rotated to coincide with
    itself) is added to the ObjectPedia and objects with no rotational symmetry are used in this
    task. A sample prompt is 'Rotate the red heart clockwise 120 degrees.'.

    where the heart is the object to be dragged and 120 denotes the angle (in degrees) to be
    rotated. 'clockwise' here is to indicate the direction of rotation. We set the angle to [1/6 *
    pi, 11/6* pi] with a step size of 1/6 *pi. The epsilon of rotation of the environment is set to
    15 degrees. And we modify the oracle to support rotational demonstration.
    """
