...
class ManipulateOldNeighbor(BaseTask):
    """Tasks requiring memory: pick an object into object, and then put its old neighbor into that
    object."""
    ...
    def __init__(
        ...
        super().__init__(
            prompt_template=(
                "First put {dragged_obj} into {base_obj} then put the object that was previously"
                " at its {direction} into the same {base_obj}."
            ),
            task_meta=task_meta,
    ...
    def _reset_prompt(self):
        ...
        while True:
            ...
            self.sampled_neighbor_idx = (
                self.sampled_dragged_obj_idx[0] + neighbor_sample_space[sampled_neighbor_dir][0],
                self.sampled_dragged_obj_idx[1] + neighbor_sample_space[sampled_neighbor_dir][1],
            )
        ...
        for i in range(self.task_meta["num_null_distractors"]):
            while True:
                ...
                if is_valid_null_distractor(
                    ...
                    self.sampled_null_distractor_indices.append(sampled_null_distractor_idx)
                    break
    ...
    def reset(self, env):
        ...
        sampled_combos = [
            ...
            for e in self.rng.choice(
                possible_dragged_combo,
                replace=False,
                size=self.task_meta["num_array_rows"] * self.task_meta["num_array_columns"],
            )
        ]
    ...
    def update_goals(self, skip_oracle=True):
        ...
        for goal_index in range(len(self.goals)):
            ...
            for i in range(len(objs)):
                ...
                pose = p.getBasePositionAndOrientation(object_id, physicsClientId=self.client_id)
