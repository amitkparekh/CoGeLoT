...
class PickInOrderThenRestore(BaseTask):
    ...
    def _update_task_meta(self):
        ...
        pos_y_interval = (y_bound[1] - y_bound[0]) / (1 + (self.task_meta["num_array_columns"]))
    ...
    def reset(self, env):
        ...
        distractor_nonempty_base_pos_centers = sampled_pos_centers[
            num_target_base_obj + 1 : num_target_base_obj + 1 + num_distractor_dragged_obj
        ]
        ...
        sampled_distractor_bases = sampled_bases[
            num_target_base_obj + 1 : num_target_base_obj + num_distractor_dragged_obj + 1
        ]
        sampled_empty_bases = sampled_bases[num_target_base_obj + num_distractor_dragged_obj + 1 :]
        ...
        sampled_base_target_textures = sampled_base_textures[1 : num_target_base_obj + 1]
        ...
        _scale = 2 if sampled_obj_base in [ObjPedia.BOWL.value, ObjPedia.PALLET.value] else 0.5
        dragged_obj_pose = (
            (
                dragged_obj_base_pos_center[0],
                dragged_obj_base_pos_center[1],
