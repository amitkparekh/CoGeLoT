...
class FollowOrder(BaseTask):
    """One shot imitation task: Stack objects in this order {video frames} In this task, the agent
    is required to imitate the moves in the given one shot video trajectory.
        ...
        while not q.empty():
            ...
            for move in possible_moves:
                if objs_at_base_position[move[0]] == 0:
                    ...
                    q.put((tuple(new_objs_at_base_position), depth + 1, new_op_sequence))
    ...
    def reset(self, env):
        ...
        if pos_y_interval <= block_sampled_size[0]:
            raise ValueError(
                "ERROR: WORKSPACE CANNOT ACCOMEDATE ALL THE OBJECTS. Please decrease the number of"
                " objects."
            )
        ...
        for i, (from_idx, to_idx) in enumerate(sampled_move_seq):
            ...
            frames_of_block_poses_in_scene.append((block_poses_in_scene, block_textures_in_scene))
        ...
        if num_distractor > 0:
            ...
            for i in range(self.REJECT_SAMPLING_MAX_TIMES + num_distractor):
                ...
                texture = self.rng.choice([t for t in self.possible_dragged_obj_texture]).value
                obj_id, _, _ = self.add_object_to_env(
        ...
        def create_scene_fn(scene_render_env: SceneRenderEnv, sim_id: int, frame_idx: int):
            (
                block_poses_frame_i,
    ...
    def move_blocks_from_to(
        ...
        """Calculate poses after a move operation block base idx starts from 0.
        ...
        new_pose_height = len(new_block_poses_in_scene[to_idx]) * 2 * half_height + half_height
        ...
        new_block_textures_in_scene[to_idx].append(new_block_textures_in_scene[from_idx].pop())
