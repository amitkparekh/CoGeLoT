...
class FollowMotion(BaseTask):
    """One shot imitation task: Follow the motion of the object {video frames} In this task, the
    agent is required to imitate the moves in the given one shot video trajectory.

    Here we define video trajectory by variable length of frames of image. This task is one of the
    tasks that have strict requirements on the manipulation sequence. The agent is required to
    precisely follow a leader object's motion, which is presented in the prompt frames. And here we
    define the motion as a random walk along a polygon that approximates a circle.
    """
    ...
    def __init__(
        ...
        assert (
            ...
            "`placeholder_scene_views` must be a str, list of str, or None, "
    ...
    def all_motion_point_coordinates(self) -> list:
        ...
        for i in range(self.num_possible_motion_points):
            new_x = (
                math.cos(i * 2 * math.pi / self.num_possible_motion_points) * self.circle_radius
            )
            new_y = (
                math.sin(i * 2 * math.pi / self.num_possible_motion_points) * self.circle_radius
            )
    ...
    def _reset_prompt(self):
        ...
        self.prompt_template = (
            "Follow this motion for {dragged_obj}: "
            + " ".join(["{{frame_{i}}}".format(i=i) for i in range(self.task_meta["num_frames"])])
            + "."
        )
    ...
    def reset(self, env):
        ...
        sampled_dragged_obj_texture = self.rng.choice(self.possible_dragged_obj_texture).value
        ...
        dragged_entries_archive.append((sampled_dragged_obj, sampled_dragged_obj_texture))
        ...
        for i in range(self.REJECT_SAMPLING_MAX_TIMES):
            ...
            if _valid_distractor(sampled_distractor_obj, sampled_distractor_obj_texture):
    ...
    def check_success(self, *args, **kwargs) -> ResultTuple:
        ...
        if all_achieved:
            ...
            for goal in self.goals:
                ...
                distances.append(
                    np.linalg.norm(np.float32(obj_pose[0][:2]) - np.float32(bases[0][0][:2]))
                )
            is_failure = any(failures) or (self._n_goals_before - self._n_goals_after == 0)
            return ResultTuple(
                success=False,
                failure=is_failure,
