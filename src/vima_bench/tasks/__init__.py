...
ALL_TASKS = {
    f"{group}/{task.task_name}": task for group, tasks in _ALL_TASKS.items() for task in tasks
}


_ALL_TASK_SUB_NAMES = [task.task_name for tasks in _ALL_TASKS.values() for task in tasks]
...
PARTITION_TO_SPECS: dict[str, dict[str, Any]] = {
