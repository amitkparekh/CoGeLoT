...
class PromptRenderer(gym.Wrapper):
    ...
    def get_prompt_img_from_text(
        ...
        lang_textsize = cv2.getTextSize(text, self._font, self._font_scale, self._font_weight)[0]
    ...
    def get_multi_modal_prompt_img(self):
        ...
        if len(prompt_assets) > 0:  # multi-modal prompt
            for idx, text_piece in enumerate(prompt_pieces[1:]):
                img_placeholder = prompt_assets[prompt_placeholders[idx]]["rgb"]["front"]
    ...
    def _resize_img_to(
        ...
        if height and width:
            ...
            if preserve_apsect_ratio:
                ...
                img = cv2.resize(img, (scaled_width, height), interpolation=cv2.INTER_AREA)
            else:
                img = cv2.resize(img, (original_w, height), interpolation=cv2.INTER_AREA)
        else:
            if preserve_apsect_ratio:
                scaled_height = int(img.shape[0] / img.shape[1] * width)
                img = cv2.resize(img, (width, scaled_height), interpolation=cv2.INTER_AREA)
            else:
